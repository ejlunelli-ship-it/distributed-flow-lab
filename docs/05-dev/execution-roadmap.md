...
| **S2 — Realtime streaming** | `SimulationHub` (per-`simulationId` groups), `ReceiveSimulationEvent` / `SimulationStateChanged`, SignalR client + Zustand store on the SPA | Epic 4 (P0) | Client receives events in `sequence` order; P95 emit→render ≤ 250 ms (NFR-1) |
