...
| **S3 — Canvas editor** ✅ | React Flow shell (pan/zoom, minimap, controls) controlled by a Zustand `canvasStore`; `NodeType` palette (drag or click to place) grouped from a data-driven `NODE_CATALOG`; edge creation with a curated directional connection matrix + inline rejection reasons (ADR-016); data-driven node/edge inspector with client-side config validation; design tokens (semantic + per-`NodeType` accents, light/dark) and Design-mode app shell + routing (`/editor/new`) | Epic 1 (P0) | User composes `Producer→Exchange→Queue→Consumer` and illegal edges (e.g. `Consumer→Producer`) are rejected inline — proven by a Playwright e2e; 41 Vitest unit/component tests green; typecheck, ESLint, Prettier and `npm run build` clean |
